...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    const fetchPrice = async () => {
      try {
        const response = await axios.get("https://api3.binance.com/api/v3/ticker/price?symbols=[\"SOLUSDC\"]");
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white dark:bg-[#0A0F1C] rounded-xl p-6 border-4 border-yellow-400">
              ...
              <p className="text-3xl font-bold text-purple-500">${solPrice.toFixed(2)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-[#0A0F1C] rounded-xl p-6 border-4 border-blue-400">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">{stakingInfo.ethStakeAmount.toFixed(4)} ETH</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Points Earned</span>
                  <span className="font-bold text-green-500">{stakingInfo.ethPoints}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Value</span>
                  <span className="font-bold">${(stakingInfo.ethStakeAmount * solPrice).toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#0A0F1C] rounded-xl p-6 border-4 border-green-400">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Staked Amount</span>
                  <span className="font-bold">{stakingInfo.solStakeAmount.toFixed(4)} SOL</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Points Earned</span>
                  <span className="font-bold text-green-500">{stakingInfo.solPoints}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 dark:text-gray-400">Value</span>
                  <span className="font-bold">${(stakingInfo.solStakeAmount * solPrice).toFixed(2)}</span>
                </div>
              </div>
            </div>
          </div>
